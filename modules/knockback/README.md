...
## Description
Launch opponents into the void in a high-knockback brawl.
...
- Fast-paced combat with minimal setup.
- Works with standard arena bounds and spawns.
...
- `/baa game <id> add micro knockback` — Attach this microgame module to the arena.
- `/baa stick` — Get the setup tool to select regions.
- `/baa game <id> knockback bounds set` — Save the game bounds for this arena.
- `/baa game <id> knockback spawn add` — Add spawn points for players.
- `/baa game <id> knockback time <minutes>` — Set the match duration.

### Module-specific steps
Finish the setup with the commands below:
- `/baa game <id> knockback setmode <last_standing|most_kills>` — Choose the win condition.

## Technical details
- **Microgame ID:** `knockback`
