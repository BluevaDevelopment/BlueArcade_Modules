...
function M.handleSnowBreak(session, handle)
  statsService.recordSnowBreak(session, handle)
end
...
local function startGameTimer(session)
  ...
  session.scheduler.runTimer("arena_" .. session.arenaId .. "_spleef_timer", function()
    if session.state.ended then
      session.scheduler.cancelTask("arena_" .. session.arenaId .. "_spleef_timer")
      return
    end
