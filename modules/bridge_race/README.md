...
- `/baa game <id> add micro bridge_race` — Attach this microgame module to the arena.
- `/baa stick` — Get the setup tool to select regions.
- `/baa game <id> bridge_race bounds set` — Save the game bounds for this arena.
- `/baa game <id> bridge_race spawn add` — Add spawn points for players.
- `/baa game <id> bridge_race time <minutes>` — Set the match duration.

### Module-specific steps
Finish the setup with the commands below:
- `/baa game <id> bridge_race finishline` — Configure the module-specific finishline setup data.

## Technical details
- **Microgame ID:** `bridge_race`
