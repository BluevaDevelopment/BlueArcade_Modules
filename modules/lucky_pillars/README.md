...
- `/baa game <id> add mini lucky_pillars` — Attach this minigame module to the arena.
- `/baa stick` — Get the setup tool to select regions.
- `/baa game <id> lucky_pillars bounds set` — Save the game bounds for this arena.
- ~~`/baa game <id> lucky_pillars spawn add`~~ — Not used in Lucky Pillars.
  Use **`/baa game <id> lucky_pillars team spawn add`** and **`/baa game <id> lucky_pillars team spawn set <team_id>`** to configure team spawns.
- `/baa game <id> lucky_pillars time <minutes>` — Set the match duration.

### Module-specific steps
Finish the setup with the commands below:
- `/baa game <id> lucky_pillars team count <value>` — Set the number of teams.
- `/baa game <id> lucky_pillars team size <value>` — Set the players per team.
- `/baa game <id> lucky_pillars team spawn add` — Add a spawn point to the next free slot (run once per team).
- `/baa game <id> lucky_pillars team spawn set <team_id>` — Overwrite the spawn point for a specific team (1, 2, ..., N).
- `/baa game <id> lucky_pillars team spawn remove <team_id>` — Remove the spawn point for a specific team (1, 2, ..., N).
- `/baa game <id> lucky_pillars region set` — Select and save the regeneration region.
