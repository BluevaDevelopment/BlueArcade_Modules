...
- Store with kits and cage-skin selection, fully wired to the shared StoreAPI.
...
- `/baa game <id> add mini skywars` — Attach this module to the arena.
- `/baa stick` — Get the setup tool to select regions.
- `/baa game <id> skywars bounds set` — Save the game bounds for this arena.
- ~~`/baa game <id> skywars spawn add`~~ — Not used in SkyWars.
  Use **`/baa game <id> skywars team spawn add`** and **`/baa game <id> skywars team spawn set <team_id>`** to configure team spawns.
- `/baa game <id> skywars time <minutes>` — Set the match duration.

### Module-specific steps
Finish the setup with the commands below:
- `/baa game <id> skywars team count <value>` — Set the number of teams.
- `/baa game <id> skywars team size <value>` — Set the players per team.
- `/baa game <id> skywars team spawn add` — Add a spawn point to the next free slot (run once per team).
- `/baa game <id> skywars team spawn set <team_id>` — Overwrite the spawn point for a specific team (1, 2, ..., N).
- `/baa game <id> skywars team spawn remove <team_id>` — Remove the spawn point for a specific team (1, 2, ..., N).
- `/baa game <id> skywars region set` — Select and save the regeneration region.
...
### Chest loot
- `bluearcade.skywars.votes.chests.basic`
- `bluearcade.skywars.votes.chests.normal`
- `bluearcade.skywars.votes.chests.op`
- `bluearcade.skywars.votes.chests.*`

The overpowered tier uses the option name `op` in its permission node. The legacy singular form
(`bluearcade.skywars.votes.chest.*`) is still accepted as a fallback for compatibility.
