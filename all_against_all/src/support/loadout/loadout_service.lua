...
function M.handleKillRegeneration(session, killer)
  local healAmount = session.config.getDouble("combat.kill_regeneration.health", 8.0)
  if healAmount <= 0 then return end

  session.player.setHealth(killer, session.player.maxHealth(killer))
end
