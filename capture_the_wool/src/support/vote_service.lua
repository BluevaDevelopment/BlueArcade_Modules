...
local WAITING_ITEM_ID = "capture_the_wool_vote_settings"
local VOTE_PERMISSION_BASE = "bluearcade.capture_the_wool.votes"
...
local MAIN_MENU_BUTTONS = {
  ...
  { category = "weather", slot = 15, material = "WATER_BUCKET", nameKey = "votes.menu.main.weather_name" },
}
...
-- instance fields (one CaptureTheWoolVoteService per module, not per arena).
...
function M.applyPendingVotes(session)
  ...
  for _, handle in ipairs(session.players()) do
    for category in pairs(CATEGORY_OPTIONS) do
...
local function buildMainMenuItems(session, handle, voteState)
  ...
  for _, button in ipairs(MAIN_MENU_BUTTONS) do
    ...
    items[#items + 1] = {
      ...
      actions = { "MODULE;capture_the_wool;menu " .. button.category },
    }
  end
...
local function buildCategoryMenuItems(session, handle, voteState, category)
  ...
  for _, entry in ipairs(CATEGORY_MENU_ITEMS[category]) do
    ...
    items[#items + 1] = {
      ...
      actions = { "MODULE;capture_the_wool;vote " .. category .. " " .. entry.option },
    }
  end
  ...
  items[#items + 1] = {
    ...
    actions = { "MODULE;capture_the_wool;menu main" },
  }
...
local function openMenu(session, handle, voteState, menuId)
  local title
  local items
  if menuId == "hearts" or menuId == "time" or menuId == "weather" then
    ...
    title = translate(session, handle, "votes.menu.main.title") or "Capture The Wool - Votes"
...
-- through - mirrors legacy CaptureTheWoolGame.handleVoteCommand's own context-or-not routing.
