-- Mirrors legacy LuckyPillarsSetup.java.
local M = {}
...
local function handleTeamSpawnAdd(ctx)
  local teamCount = ctx.data.getInt("teams.count", 0)
  if teamCount <= 0 then
  ...
  local nextSlot = nil
  for i = 1, teamCount do
    if not ctx.data.has("game.play_area.team_spawns." .. i) then
      nextSlot = i
      break
    end
  end
  ...
  if not nextSlot then
    ctx.reply(setupMessage("team_spawn.all_slots_filled"):gsub("{count}", tostring(teamCount)))
    return
  end

  ctx.data.setLocation("game.play_area.team_spawns." .. nextSlot, ctx.playerLocation)
  ctx.data.save()

  ctx.reply(setupMessage("team_spawn.added")
    :gsub("{slot}", tostring(nextSlot))
    :gsub("{count}", tostring(teamCount)))
end

local function handleTeamSpawnSet(ctx)
  if ctx.argCount < 3 then
    ctx.reply(setupMessage("team_spawn.usage_set"))
    return
  end
...
local function handleTeamSpawn(ctx)
  ...
  if action == "add" then
    handleTeamSpawnAdd(ctx)
  elseif action == "set" then
    handleTeamSpawnSet(ctx)
  elseif action == "remove" then
    handleTeamSpawnRemove(ctx)
  else
    ctx.reply(setupMessage("team_spawn.usage_add"))
    ctx.reply(setupMessage("team_spawn.usage_set"))
    ctx.reply(setupMessage("team_spawn.usage_remove"))
  end
end
